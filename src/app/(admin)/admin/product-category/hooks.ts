...
import { FetchProductCategory } from '@/graphql/query/product'
import {
...
const useHooks = () => {
  //get data categories
  const [getItems, { loading, data, error }] =
    useLazyQuery(FetchProductCategory)
  ...
  const actionGetItems = async () => {
    try {
      ...
      if (data?.ProductCategory?.Fetch?.data?.items.length > 0) {
        console.log(data?.ProductCategory?.Fetch?.data?.items)
        return setListData(data?.ProductCategory?.Fetch?.data?.items)
      }
